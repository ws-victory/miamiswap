...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  miaperblock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const lpTotalInQuoteToken = isTokenOnly
    ? new BigNumber(singleTokenBalanceMC).div(BIG_TEN.pow(tokenDecimals))
    : quoteTokenAmountMc.times(new BigNumber(2))

  // Only make masterchef calls if farm has pid
  const [info, totalAllocPoint, miaPerBlock] =
    pid || pid === 0
  ...
  const MiaPerBlock = miaPerBlock ? new BigNumber(miaPerBlock).div(BIG_TEN.pow(9)) : BIG_ZERO
  ...
  return {
    ...
    miaperblock: MiaPerBlock.toJSON(),
    canharvest,
  }
}
